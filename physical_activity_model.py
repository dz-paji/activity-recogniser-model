...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Flatten, Dense, Dropout, LayerNormalization, GRU
from tensorflow.keras.optimizers import Adam
from sklearn.metrics import classification_report
from tensorflow import keras
import keras_tuner as kt

your_dataset_path = "model_data/static"
window_size = 20
step_size = 20
...
def build_1d_cnn_gru(hp, input_shape, num_classes):
    model = Sequential()
    
    model.add(Conv1D(filters=hp.Int('filters', min_value=64, max_value=256, step=32),
                     kernel_size=3, 
                     activation='relu', 
                     input_shape=input_shape))
    
    model.add(MaxPooling1D(pool_size=hp.Choice('pool_size', values=[2, 3, 4])))
    model.add(GRU(units=hp.Int('GRU units', min_value=64, max_value=256, step=32), return_sequences=True))
    model.add(Flatten())
    model.add(Dense(hp.Int("dense", min_value=64, max_value=256, step=32), activation='relu'))
    model.add(Dropout(0.4))
    model.add(Dense(num_classes, activation='softmax'))
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    model.summary()
...
# Explanation:
    ...
    tuner = kt.Hyperband(
    lambda hp: build_1d_cnn_gru(hp, input_shape, num_classes),
    ...
    model = build_1d_cnn_gru(best_hps, input_shape, num_classes)
